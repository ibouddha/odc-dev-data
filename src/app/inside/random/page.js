...
const RandomChoiceSection = () => {
  ...
  const choices = [
    {
      id: 1,
      name: "qui est cette personne mystérieuse ?",
  ...
  return (
    <div className="p-4 bg-white dark:bg-slate-900 rounded-md">
      ...
      <div className="mt-10 py-4 flex gap-2 justify-center overflow-x-auto">
        <SwiperView choices={choices} />
      </div>
      <div className="mt-8 flex justify-center p-4">
        <button onClick={handleRandomChoice}>
          <svg
            width="80"
            height="80"
            viewBox="0 0 102 106"
...
const RandomChoiceCard = ({ choice, active = false }) => {
  return (
    <div
      className={clsx(
        "rounded-md  w-52 bg-slate-900 dark:bg-white pb-10 px-6 ",
        active
          ? "opacity-100 transition-all duration-300 -translate-y-4"
          : "opacity-50"
      )}
    >
...
const SwiperView = ({ choices = [] }) => {
  const [mySwiper, setMySwiper] = useState({});

  const lenChoices = choices.length;
  const leftChoice = choices.slice(0, lenChoices / 2);
  const rightChoice = choices.slice(lenChoices / 2, lenChoices);
  const defaultChoice = {
    id: 0,
    name: "qui est cette personne mystérieuse ?",
    image: "/images/defaultchoise.png",
    active: true,
  };
